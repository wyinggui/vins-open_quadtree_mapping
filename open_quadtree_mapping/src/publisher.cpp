...
void quadmap::Publisher::publishDepthmap(ros::Time msg_time)
{
  ...
  std::cout << "1" << std::endl;//addtest0710
  min = 0.5; max = 10;
  // min = 0; max = 65;
  // min = 10; max = 30;
  depthmap_mat.convertTo(adjMap,CV_8UC1, 255/(max-min), -min);
  if (wyg==200)
 { cv::imwrite("/home/wyinggui/output.bmp",depthmap_mat);
  cv::imwrite("/home/wyinggui/output1.bmp",adjMap);
  cv::imwrite("/home/wyinggui/outputraw.bmp",reference_mat);
  }
  if (wyg==300)
 { cv::imwrite("/home/wyinggui/output_300.bmp",depthmap_mat);
  cv::imwrite("/home/wyinggui/output1_300.bmp",adjMap);
  cv::imwrite("/home/wyinggui/outputraw_300.bmp",reference_mat);
  }
  //std::cout << "1.3" << std::endl;//addtest0710
   /*cv::Mat falseColorsMap;
  std::cout << "1.4" << std::endl;//addtest0710
 cv::applyColorMap(adjMap, falseColorsMap, cv::COLORMAP_JET);//cv::COLORMAP_RAINBOW);
  std::cout << "1.5" << std::endl;//addtest0710
  cv::Mat mask;
  ...
  // cv::cvtColor(reference_mat, black_image, cv::COLOR_GRAY2BGR);
  // cv::addWeighted( black_image, 0.5, falseColorsMap, 0.5, 0.0, falseColorsMap);
  std::cout << "2" << std::endl;//addtest0710
  ...
  if(nh_.ok())
  {
    ...
    std::cout << "4" << std::endl;//addtest0710
  }
}
...
/*void quadmap::Publisher::publishDepthmap(ros::Time msg_time)
{
  ...
  cv::Mat falseColorsMap;
  std::cout << "1.4" << std::endl;//addtest0710
  //adjMap=cv::imread("/home/wyinggui/output1.bmp");
  cv::applyColorMap(adjMap, falseColorsMap, cv::COLORMAP_JET);//cv::COLORMAP_RAINBOW);
  std::cout << "1.5" << std::endl;//addtest0710
  cv::Mat mask;
  cv::inRange(falseColorsMap, cv::Scalar(0, 0, 255), cv::Scalar(0, 0, 255), mask);
